...
def msplat_render(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, scaling_modifier = 1.0, override_color = None):
    position = pc.get_xyz
    ...
    intr_params = torch.tensor([fx, fy, cx, cy]).cuda().float()
    extr_params = viewpoint_camera.world_view_transform.transpose(0, 1)
    extr_params = extr_params[:3, :]
    ...
    sh2rgb = msplat.compute_sh(shs, direction, visible)
