...
def naive_gauss_noise_bounded(args):
    fix_all_random_seed()
    ...
    for attack_iter in range(1, adv_iters + 1):
        ...
        # Add gaussian noise and clamp into the epsilon ball
        clean_gt_image = viewpoint_cam.original_image
        adv_gt_image = clean_gt_image.detach().clone()
        gauss_noise = torch.randn_like(adv_gt_image)
        adv_image_unclipped = adv_gt_image.data + gauss_noise
        clipped_perturbation = torch.clamp(adv_image_unclipped - clean_gt_image, -adv_epsilon, adv_epsilon) # clip into epsilon ball
        adv_gt_image = torch.clamp(clean_gt_image + clipped_perturbation, 0, 1)
        viewpoint_cam.set_adv_image(adv_gt_image)
        print(f"Naive gauss-noise attack view {attack_iter} done")
...
if __name__ == '__main__':
    ...
    naive_gauss_noise_bounded(args)
